...
-- me devuelva la cantidad de ciudadanos que tengo en un pais siempre y cuando exista ese pais

-- VARIABLES

SET @pais = "%alemania%";
SELECT @pais FROM DUAL;

SELECT 
	COUNT( DISTINCT ci.id_ciudadano) AS total_ciudadanos
...
BEGIN
	...
	SELECT 
	COUNT( DISTINCT ci.id_ciudadano) INTO valor_retorno
	FROM funciones.pais AS p
	LEFT JOIN funciones.ciudad AS c 
		ON p.id_pais = c.id_pais
	LEFT JOIN funciones.ciudadano AS ci
	...
	IF valor_retorno = 0 THEN
		SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'No citizens found for the specifiqued country', MYSQL_ERRNO = 1000;
	END IF;
...
-- Una funcion que dependiendo del pais devolve em documento del ciudadano con mayor edad
...
BEGIN
    ...
    -- Extraer los primeros dos digitos del DNI
    ...
    -- DNIs que empiezan con 2-3: nacidos después de 1970-1999

    CASE  -- Faltaba "CASE" aquí
    ...
    END CASE;  -- Cerramos el bloque con "END CASE;"
...
SELECT 
	p.id_pais,
	MAX(fx_calculo_edad(cd.num_doc)) AS Maximo_edad
FROM funciones.pais AS p
LEFT JOIN funciones.ciudad AS c 
	ON p.id_pais = c.id_pais
LEFT JOIN funciones.ciudadano AS ci
