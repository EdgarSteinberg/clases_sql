...
CREATE TABLE ciudadano (
    ...
    id_ciudad INT , 
	CONSTRAINT solo_email CHECK (email LIKE '%@emai%'),
    CONSTRAINT solo_upper CHECK (nombre REGEXP '[A-Z]+')
);
...
-- España - Barcelona
('Pablo Torres', 'pablo.torres@gemail.com', 2),
('Laura Navarro', 'laura.navarro@gemail.com', 2),
...
UPDATE integridad.ciudadano
	SET email = REPLACE(email, 'carlos.martinez', 'ana.garcia')
    WHERE id_ciudadano = 1;    

UPDATE integridad.ciudadano
	SET email = REPLACE(email, 'gmail', 'email')
    WHERE id_ciudadano = 1;    
    
 UPDATE integridad.ciudadano
	SET email = REPLACE(email, 'ana.garcia@email.com', 'carlos.martinez@email.com')
    WHERE id_ciudadano = 1;    
