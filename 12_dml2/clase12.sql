...
UPDATE donaton.beneficiario 
SET 
    direccion = 'Azucar 1987',
    nombre_encargado = 'Edgar Stein',
    fecha_creacion = STR_TO_DATE('10/10/1987', '%d/%m/%Y')
WHERE
    nombre_encargado LIKE 'Edgar%';
        
UPDATE donaton.beneficiario 
SET 
    nombre_encargado = 'Edgar Steinberg'
WHERE
    id_beneficiario = 3;
...
-- como hacemos para insetar data de otra forma
-- Ingestar datos voluntarios
...
SET SQL_SAFE_UPDATES = FALSE;
UPDATE donaton.voluntario AS v,
    _update_data AS ud 
SET 
    v.fecha_nacimiento = ud.fecha_nacimiento
WHERE
    v.nombre = ud.nombre
        AND v.fecha_nacimiento = DATE_ADD(ud.fecha_nacimiento,
        INTERVAL 25 YEAR);
...
CREATE TABLE sectorizado_voluntarios (
...
SELECT SUBSTRING_INDEX(nombre,' ',-1) FROM donaton.voluntario;


DELETE FROM donaton.voluntario
WHERE EXISTS (
